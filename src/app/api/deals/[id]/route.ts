...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  if (status === 'FUNDED') {
    const emailTemplate = emailTemplates.dealFunded(
      seller.name, deal.item_name, deal.amount_usdc, dealLink
    )
  ...
  if (status === 'COMPLETED') {
    const emailTemplate = emailTemplates.dealCompleted(
      seller.name, deal.item_name, deal.amount_usdc
    )
    await sendEmail({ to: seller.email, ...emailTemplate })
    // Also notify buyer
    await sendEmail({
  ...
  if (status === 'DISPUTED') {
    const emailTemplate = emailTemplates.disputeOpened(seller.name, dealLink)
    await sendEmail({ to: seller.email, ...emailTemplate })
    await sendEmail({ to: deal.buyer_email, ...emailTemplates.disputeOpened('there', dealLink) })
  }
